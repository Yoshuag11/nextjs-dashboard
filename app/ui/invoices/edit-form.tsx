...
import { FormError } from "@/app/ui/invoices/form-error";
import { updateInvoice } from "@/app/lib/action";
import type { StateInterface } from "@/app/lib/action";
export default function EditInvoiceForm({
  ...
  const { amount, customer_id, id, status } = invoice;
  const initialState: StateInterface = {};
  const updateInvoiceIdBound = updateInvoice.bind(null, id);
  const [state, formAction] = useActionState(
    updateInvoiceIdBound,
    initialState
  );
...
-translate-y-1/2 text-gray-500"
          ...
          <FormError errors={errors?.customerId} id="customer-error" />
        </div>
        {/* Invoice Amount */}
        <div className="mb-4">
...
w-[18px] -translate-y-1/2 text-gray-500 peer-focus:text-gray-900"
          ...
          <FormError errors={errors?.amount} id="amount-error" />
        </div>
        {/* Invoice Status */}
        <fieldset>
...
text-gray-600 focus:ring-2"
                ...
                <label
                  htmlFor="paid"
                  className="ml-2 flex cursor-pointer items-center gap-1.5 rounded-full \
bg-green-500 px-3 py-1.5 text-xs font-medium text-white"
                >
          ...
          <FormError errors={errors?.status} id="status-error" />
        </fieldset>
        <FormError errors={message} id="form-errors" />
      </div>
